...
class OmniPathVLM(VLM):
    def __init__(
        ...
        self.all_module_keys = ["vision_backbone", "llm_backbone", "projector", "bbox_encoder", "bbox_decoder", "mask_encoder", "mask_decoder"]
        ...
        tokenizer.add_tokens([DEFAULT_IMAGE_TOKEN, DEFAULT_BBOX_TOKEN, DEFAULT_MASK_TOKEN])
        self.llm_backbone.llm.resize_token_embeddings(len(tokenizer), pad_to_multiple_of=64)
        self.config.image_id, self.config.bbox_id, self.config.mask_id = tokenizer.convert_tokens_to_ids([DEFAULT_IMAGE_TOKEN, DEFAULT_BBOX_TOKEN, DEFAULT_MASK_TOKEN])
        input_embeddings = self.llm_backbone.llm.get_input_embeddings().weight.data
        input_embeddings[self.config.image_id] = self.config.image_id # 设置为常量值，反正后面也会替换为实际embedding
        input_embeddings[self.config.bbox_id] = self.config.bbox_id # 设置为常量值，反正后面也会替换为实际embedding
        input_embeddings[self.config.mask_id] = self.config.mask_id # 设置为常量值，反正后面也会替换为实际embedding
    ...
    def from_pretrained(
        ...
        if "mask_encoder" in model_state_dict:
            vlm.mask_encoder.load_state_dict(model_state_dict["mask_encoder"])
        if "mask_decoder" in model_state_dict:
            vlm.mask_decoder.load_state_dict(model_state_dict["mask_decoder"])
    ...
    def freeze_backbones(self, stage: str) -> None:
        ...
        if stage == "align":
            ...
            self.mask_encoder.requires_grad_(True)
            self.mask_decoder.requires_grad_(True)

            # Add to `self.trainable_module_keys`
            self.trainable_module_keys = ["projector", "bbox_encoder", "bbox_decoder", "mask_encoder", "mask_decoder"]
            ...
            # Add to `self.trainable_module_keys`
            self.trainable_module_keys = ["projector", "llm_backbone", "bbox_encoder", "bbox_decoder", "mask_encoder", "mask_decoder"]
            ...
            # Add to `self.trainable_module_keys`
            self.trainable_module_keys = ["vision_backbone", "projector", "bbox_encoder", "bbox_decoder", "mask_encoder", "mask_decoder"]
            ...
            # Add to `self.trainable_module_keys`
            self.trainable_module_keys = ["vision_backbone", "projector", "llm_backbone", "bbox_encoder", "bbox_decoder", "mask_encoder", "mask_decoder"]
    ...
    def load_from_checkpoint(self, stage: str, run_dir: Path, pretrained_checkpoint: Optional[Path] = None) -> None:
        ...
        if pretrained_checkpoint is not None:
            ...
            if "mask_encoder" in model_state_dict:
                self.mask_encoder.load_state_dict(model_state_dict["mask_encoder"])
            if "mask_decoder" in model_state_dict:
                self.mask_decoder.load_state_dict(model_state_dict["mask_decoder"])
    ...
    def forward(
        ...
        masks: Optional[List[torch.FloatTensor]] = None, # List of Tensors with shape [N, H, W], each data have different N 
        ...
        if bbox_valid_flag or bbox_numbers.sum() > 0:
            ...
            # bboxes = torch.rand(1, 4).round(decimals=3).view(1,2,2).sort(dim=1)[0].view(-1, 4).to(inputs_embeds)
            # bbox_embeds = self.bbox_encoder(bboxes)
        ...
        # Encoder masks
        mask_numbers = (input_ids == self.config.mask_id).sum(1)
        mask_valid_flag = masks is not None and any(m.shape[0]>0 for m in masks)
        if mask_valid_flag or mask_numbers.sum() > 0:
            assert all([len(m) == mask_numbers[i] for i, m in enumerate(masks)]), f'{[(len(m), mask_numbers[i]) for i, m in enumerate(masks)]}'
            masks = torch.cat(masks, 0).unsqueeze(1) # [B, 1, H, W]
            mask_embeds = self.mask_encoder(masks)
            inputs_embeds[input_ids == self.config.mask_id] = mask_embeds
        else:
            masks = torch.zeros(1, 1, self.vision_backbone.default_image_size, self.vision_backbone.default_image_size).to(inputs_embeds)
            mask_embeds = self.mask_encoder(masks)
        ...
        if len(multimodal_indices) == 0:
            # Assume that no bboxes and no masks in pure text 
            assert bbox_numbers.sum() == 0 and mask_numbers.sum() == 0
        ...
        loss = outputs.loss
        if labels is not None:
            # TODO: Currently, the scenario where human input includes bboxes and masks has not been considered.
            ...
            # [B, 1, D] [B, HW, D] => [B, 1, H, W]
            mask_preds = self.mask_decoder(mask_hs if mask_valid_flag else mask_embeds.unsqueeze(1),
                                           patch_features.repeat_interleave(mask_numbers, 0) if mask_valid_flag 
                                           else torch.zeros(1,self.vision_backbone.num_patches,self.vision_backbone.embed_dim).to(inputs_embeds))
            loss += self.mask_loss(mask_preds, masks) * (1 if mask_valid_flag else 0) # mask是生成的则不参与loss计算
    ...
    def prepare_inputs_for_generation(
        ...
        **kwargs: Dict[str, Any],
        ...
        """所有在generate时需要的参数都需要包含在model_inputs内"""
        if past_key_values:
    ...
    def generate(self, image: Image, prompt_text: str, **kwargs: str) -> str:
        ...
        # restore bounding boxes and masks
        generated_ids = outputs.sequences[0, input_ids.shape[1] :]
        generated_text = tokenizer.decode(generated_ids, skip_special_tokens=True).strip()
        if torch.isin(generated_ids, torch.tensor([self.config.bbox_id, self.config.mask_id]).to(generated_ids)).any():
        ...
        # for mask
        replace_indices = torch.isin(generated_ids, torch.tensor([self.config.mask_id]).to(generated_ids))
        if replace_indices.sum() > 0:
            with torch.autocast("cuda", dtype=autocast_dtype, enabled=self.enable_mixed_precision_training):
                patch_features = self.vision_backbone(pixel_values)
                mask_preds = self.mask_decoder(last_hidden_states[replace_indices].unsqueeze(0), patch_features) # [1, N, H, W]
            mask_preds = (F.sigmoid(mask_preds.squeeze(0)) > 0.5).byte().cpu().numpy()
            generated_text = restore_masks(generated_text, mask_preds)
