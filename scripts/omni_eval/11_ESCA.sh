...
RES_FILE="./playground/omnipath/eval/$CKPT_NAME/11_ESCA.log"

mkdir -p ./playground/omnipath/eval/$CKPT_NAME

QUESTION_FILE="/c22073/datasets/ESCA/question_test.jsonl"
ANSWER_FILE="./playground/omnipath/eval/$CKPT_NAME/ESCA_test.jsonl"

if [[ " $@ " != *" -ng "* ]]; then
  python prismatic/eval/model_vqa.py \
      --model_path $CKPT_DIR \
      --question_file $QUESTION_FILE \
      --image_folder /c22073/datasets/ESCA/ \
